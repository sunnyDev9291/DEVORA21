...
function parsePlatform(value: unknown, fallback: JobCrawlPlatform): JobCrawlPlatform {
  return value === "hiringcafe" || value === "builtin" ? value : fallback;
}
...
type CrawlConfig = {
  ...
  path: "/jobs/crawl/builtin" | "/jobs/crawl/hiringcafe";
  label: string;
...
/** Route to Built In or HiringCafe crawl based on listing URL host. */
export async function crawlJobs(url: string, signal?: AbortSignal): Promise<JobCrawlResult> {
  ...
  throw new ApiError("URL must be a builtin.com or hiringcafe.com listing.", 400);
}
