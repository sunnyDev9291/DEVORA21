  ...
  return (
    <div className="mt-4 rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-4 text-sm text-amber-100">
      <p>
        ...
        Use <span className="font-semibold text-white">Sign up with Google</span> above only if
      ...
      <div className="mt-3 flex flex-col gap-2 sm:flex-row">
        ...
        <button
          ...
          disabled={signingOut}
          className="inline-flex items-center justify-center rounded-xl border border-white/10 px-3 py-2 text-xs font-semibold text-slate-200 hover:bg-white/[0.05] disabled:opacity-50"
        >
          {signingOut ? "Signing out…" : "Sign out to use email signup"}
        </button>
        <Link
          href={AUTH_LINKS.login}
          className="inline-flex items-center justify-center rounded-xl px-3 py-2 text-xs font-semibold text-amber-200 underline underline-offset-2 hover:text-white"
        >
          Sign in page
        </Link>
      </div>
