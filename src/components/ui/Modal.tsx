  ...
  const overlay = variant === "panel" ? (
    ...
    >
      ...
      <div
        ...
        className={`pointer-events-auto fixed bottom-24 right-4 sm:right-6 w-[calc(100vw-2rem)] max-w-[400px] h-[min(560px,calc(100dvh-7rem))] bg-white/88 dark:bg-warm-900/86 border border-slate-200 dark:border-white/[0.10] rounded-2xl shadow-2xl backdrop-blur-md flex flex-col overflow-hidden animate-fade-up ${className}`}
    ...
    >
      ...
      <div
        ...
        aria-labelledby={titleId}
        aria-label={ariaLabel}
        className={`relative z-10 w-full bg-white/88 dark:bg-warm-900/86 border border-slate-200 dark:border-white/[0.10] rounded-2xl shadow-2xl backdrop-blur-md flex flex-col overflow-hidden max-h-[min(90dvh,calc(100dvh-6rem))] pointer-events-auto animate-dialog-enter ${className || "max-w-lg"}`}
        onClick={(e) => e.stopPropagation()}
      >
